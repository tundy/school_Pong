...
AiAreaMiddleV4.prototype = Object.create(Paddle.prototype);

AiAreaMiddleV4.prototype.update = function(game, delta, index)
{
        ...
        if(this.side === 0)                             // Are you on top ?
        {
            if(game.playground[i].y <= canvas.height/2)      // is ball on your half ?
            {
                if(game.playground[i].speedy < 0)            // is ball moving toward you ?
                {
                    ...
                    else if(ball.y - ball.speedy > game.playground[i].y - game.playground[i].speedy)   // is that ball the most close to you ?
                    {
                        ball = game.playground[i];           // that ball is my target
                    }
                }
